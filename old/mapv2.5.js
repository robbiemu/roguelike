...
var Dungeon = {
  ...
  rooms: [],
  Generate: function () {
    this.rooms = RoomFactory.getRooms().map(room => {
      room.x = getRandom(-1, 1)
      room.y = getRandom(-1, 1)
      return room
    })

    console.log(this.rooms.map(r => r.type))
    ...
    let mapwidth = this.rooms.reduce((p,c) => c.x+c.width>p?c.x+c.width:p, 0)+2
    let mapheight = this.rooms.reduce((p,c) => c.y+c.height>p?c.x+c.height:p, 0)+2
    ...
    console.log(this.map, this.rooms, mapwidth, mapheight, factor)
