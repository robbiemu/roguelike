...
var Dungeon = {
  ...
  SquashRooms: function (midpoint) {
		let [l, r] = (midpoint.x > midpoint.y)? 
      [midpoint.y, midpoint.x]: [midpoint.x, midpoint.y]
    let changed, chance
    do {
      changed = 0
      this.rooms.sort((a,b) => 
        Math.abs(a.x-midpoint.x) + Math.abs(a.y-midpoint.y) > 
        Math.abs(b.x-midpoint.x) + Math.abs(b.y-midpoint.y)
      ).forEach((room, j) => {
        while (true) {
          var old_position = {
            x: room.x,
          ...
          chance = Math.random() * (l+r)
          if (chance < l && room.x !== midpoint.x)
  	        room.x>midpoint.x? room.x--: room.x++
          else if (chance > l && room.y !== midpoint.y) 
          	room.y>midpoint.y? room.y--: room.y++
          if ((room.x === midpoint.x) && (room.y === midpoint.y)) {
            break
          } else { 
            changed++
          }
